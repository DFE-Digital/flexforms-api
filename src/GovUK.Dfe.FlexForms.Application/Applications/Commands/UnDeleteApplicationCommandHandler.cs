...
[RateLimit(1, 30)]
public sealed record UnDeleteApplicationCommand(Guid ApplicationId) : IRequest<Result<ApplicationDto>>, IRateLimitedRequest;

public sealed class UnDeleteApplicationCommandHandler(
    IEaRepository<Domain.Entities.Application> applicationRepo,
    IAuthenticatedUserService authenticatedUserService,
    IPermissionCheckerService permissionCheckerService,
    IUserCacheInvalidator userCacheInvalidator,
    IUnitOfWork unitOfWork) : IRequestHandler<UnDeleteApplicationCommand, Result<ApplicationDto>>
{
    public async Task<Result<ApplicationDto>> Handle(
        UnDeleteApplicationCommand request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            application.UnDelete(now, dbUser.Email, dbUser.Name, dbUser.Id!, application.PreDeletedStatus); 
